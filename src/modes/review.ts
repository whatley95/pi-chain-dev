...
async function handleFileReview(
  ...
  const { ctx, config, reviewProfile, signal, themedBg } = rc;
  const arg = reviewFile.trim();
  const filePath = isAbsolute(arg) ? arg : join(ctx.cwd, arg);

  if (!isPathUnderCwd(ctx.cwd, filePath)) {
    return {
      content: [{ type: "text" as const, text: `cdev review error: reviewFile must be inside the project workspace: ${reviewFile}` }],
  ...
  const onProgress = (stage: string, model: string) => {
    ...
    ctx.ui.setWidget("cdev-progress", [themedBg("toolPendingBg", `${icon} ${label} reviewing ${reviewFile}…  (${model})`)]);
  };
...
async function handleDiffReviewInternal(
...
async function handleSessionReviewInternal(
  p: AutoForkParamsType,
  rc: ReviewContext,
): Promise<{ content: Array<{ type: string; text: string }>; details: unknown; isError?: boolean }> {
  const { ctx, config, reviewProfile, signal, themedBg } = rc;
...
export async function handleReview(
  ...
  const rc: ReviewContext = { ctx, config, reviewProfile, signal, themedBg };

  if (typeof p.reviewFile === "string" && p.reviewFile.trim()) {
    return handleFileReview(p.reviewFile, rc);
  }
  if (typeof p.diffSpec === "string" && p.diffSpec.trim()) {
    return handleDiffReviewInternal(p, rc);
  }
  return handleSessionReviewInternal(p, rc);
}
