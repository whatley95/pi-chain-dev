...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Chain Dev",
    description:
      "Two-stage development fork: first cheap models (1 scout by default) explore and gather evidence, then a powerful model (forge) synthesizes a structured report from merged findings. Set review=true to skip exploration and run code review with the powerful model only. Set quick=true for scout only (raw findings, no forge). Set parallel=N (1-3) to use multiple parallel scouts. Set recall=<topic> to retrieve past fork findings from project memory (no fork runs). Set reviewFile=<path> with review=true to review a specific file/artifact instead of the session. Set diffSpec=<range> to review a git/svn diff (e.g. 'HEAD~3..HEAD'). When cdev auto mode is enabled, proactively use this tool for exploration tasks.",
    promptSnippet: "Two-stage fork: 1 scout (cheap) explores → forge (powerful) writes. Use parallel:N for multiple scouts. Use quick:true for scout only. Use recall to retrieve past findings.",
    promptGuidelines: [
      ...
      "CRITICAL: Never commit changes unless the user explicitly asks you to. Running cdev, tests, lint, or typecheck does NOT mean you should commit. Wait for explicit user confirmation before committing.",
    ...
    parameters: Type.Object({
      ...
      parallel: Type.Optional(Type.Integer({
        description:
          "Number of parallel scouts (1-3, default 1). Each scout runs the same task independently; findings are merged before forge. Set to 1 for single scout. Each scout can use a different model via /cdev-model. A backup scout can take over failed sub-tasks if configured via parallelBackup.",
      })),
